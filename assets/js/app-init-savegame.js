function markersSnapshot() {
    return JSON.stringify(markers.map(marker => ({ id: marker.id, name: marker.name, group: marker.group })));
}
...
const START_SCREEN_VERSION = '5.8.0';
...
function startDisabledNav(iconName, label, title = 'Nach Auswahl eines Spielstands verfügbar') {
    return `<button class="start-nav-item is-disabled" type="button" title="${startEscapeAttr(title)}">${startIcon(iconName, 'start-nav-icon')}<span>${label}</span></button>`;
}
    ...
    picker.innerHTML = `
        <aside class="start-sidebar">
            ...
            <nav class="start-nav" aria-label="Startnavigation">
                <button class="start-nav-item is-active" id="startNavHome" type="button" data-start-view="home">${startIcon('home','start-nav-icon')}<span>Start</span></button>

                <div class="start-nav-label">Hof</div>
                ${startDisabledNav('dashboard', 'Übersicht')}
                ${startDisabledNav('fields', 'Felder')}
                ${startDisabledNav('vehicles', 'Fuhrpark')}
                ${startDisabledNav('animals', 'Tiere')}

                <div class="start-nav-label">Planung</div>
                ${startDisabledNav('production', 'Produktionen')}
                ${startDisabledNav('database', 'Markt')}
                ${startDisabledNav('calendar', 'Verträge')}

                <div class="start-nav-label">AutoDrive</div>
                ${startDisabledNav('map', 'Marker', 'Nach Auswahl eines Spielstands mit AutoDrive verfügbar')}
                ${startDisabledNav('map', 'Karte', 'Nach Auswahl eines Spielstands mit AutoDrive verfügbar')}

                <div class="start-nav-divider"></div>
                ${startDisabledNav('backup', 'Backups')}
                <button class="start-nav-item" id="startNavSystem" type="button" data-start-view="system">${startIcon('system','start-nav-icon')}<span>System</span><i class="start-system-dot" id="startSystemDot"></i></button>
            </nav>
...
function renderStartSavegame(savegame) {
    const folder = String(savegame.folder || 'savegame');
    const farmName = savegame.farmName || savegame.savegameName || folder;
    const mapTitle = savegame.mapTitle || 'Unbekannte Karte';
    const manager = savegame.manager || '';
    const saveDate = savegame.saveDate || '–';
    return `
        <article class="start-save-card" data-save-folder="${startEscapeAttr(folder)}">
            ...
            <div class="start-save-body">
                ...
                <div class="start-save-actions">
                    ${savegame.hasAutoDrive ? '' : '<span class="start-ad-badge">Kein AutoDrive</span>'}
...
async function loadSavegameList() {
    ...
    try {
        const response = await fetch('api.php?action=list_savegames');
        const data = await response.json();
        if (!response.ok || data.error) throw new Error(data.error || 'Spielstände konnten nicht gelesen werden.');
...
async function selectSavegame(folder) {
    if (!folder) return;
    const response = await fetch('api.php?action=select_savegame', {
        method: 'POST',
...
async function loadMarkers() {
    ...
    const response = await fetch('api.php?action=markers');
    const data = await response.json();
    if (data.error === 'no_savegame_selected') {
        showPickerScreen();
        return;
    }
    ...
    markers.forEach(marker => { marker.uid = 'm' + (uidCounter++); });
    selectedUids.clear();
    knownGroups = [...data.groups].sort((a, b) => a.localeCompare(b, 'de', { numeric: true }));
    collapsedGroups = new Set([...knownGroups, UNGROUPED]);
...
// Kartenansicht wird in den spezialisierten Kartenmodulen implementiert.
